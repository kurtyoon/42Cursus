...
// at radix sort, hard coding
void	st_sort_small(t_list **a, t_list **b)
{
	if (*(int *)st_get_idx(*a, 1)->content == *(int *)st_min(*a)->content && \
		*(int *)st_get_idx(*a, 2)->content == *(int *)st_max(*a)->content)
		st_swap_ab(a, 'a');
	if (*(int *)st_get_idx(*a, 1)->content == *(int *)st_min(*a)->content && \
		*(int *)st_get_idx(*a, 0)->content == *(int *)st_max(*a)->content)
		st_rotate_ab(a, 'a');
	if (*(int *)st_get_idx(*a, 2)->content == *(int *)st_min(*a)->content && \
		*(int *)st_get_idx(*a, 1)->content == *(int *)st_max(*a)->content)
		st_rrotate_ab(a, 'a');
	if (*(int *)st_get_idx(*a, 0)->content == *(int *)st_min(*a)->content && \
		*(int *)st_get_idx(*a, 1)->content == *(int *)st_max(*a)->content)
	{
		st_swap_ab(a, 'a');
		st_rotate_ab(a, 'a');
